...
bool isOdd(const maksimov::Polygon& polygon)
{
  return polygon.points.size() % 2 == 1;
}

bool isEven(const maksimov::Polygon& polygon)
{
  return !isOdd(polygon);
}

bool hasNVertexes(const maksimov::Polygon& polygon, const size_t numberOfVertexes)
{
  return polygon.points.size() == numberOfVertexes;
}

double countAreaMean(double area, const maksimov::Polygon& polygon, const size_t numberOfPolygons)
{
...
void maksimov::doArea(std::istream& in, std::ostream& out, const std::vector< Polygon >& polygons)
{
...
void maksimov::doMax(std::istream& in, std::ostream& out, const std::vector< Polygon >& polygons)
{
  std::string command = "";
  in >> command;
...
void maksimov::doMin(std::istream& in, std::ostream& out, const std::vector< Polygon >& polygons)
{
  std::string command = "";
  in >> command;
...
void maksimov::doCount(std::istream& in, std::ostream& out, const std::vector< Polygon >& polygons)
{
  std::string command = "";
  in >> command;
...
bool isRight(const maksimov::Polygon& polygon)
{
  return polygon.hasRightAngle();
}

void maksimov::doRightShapes(std::ostream& out, const std::vector< Polygon >& polygons)
{
  out << std::count_if(polygons.cbegin(), polygons.cend(), isRight);
}

bool hasSamePoints(const maksimov::Point& delta, const maksimov::Point& point, const maksimov::Polygon& polygon)
{
  maksimov::Point dest = { point.x + delta.x, point.y + delta.y };

  using namespace std::placeholders;
  auto accSamePoints = std::bind(maksimov::comparePoints, dest, _1);
  return std::find_if(polygon.points.cbegin(), polygon.points.cend(), accSamePoints) != polygon.points.cend();
}

bool areSame(const maksimov::Polygon& polygon1, const maksimov::Polygon& polygon2)
{
  ...
  maksimov::Point delta = { dx, dy };

  using namespace std::placeholders;
  std::function< bool(const maksimov::Point&) > accSamePoints = std::bind(hasSamePoints, delta, _1, polygon2);
...
void maksimov::doSame(std::istream& in, std::ostream& out, const std::vector< Polygon >& polygons)
{
