...
void ponomarev::outputInfoAboutCommands(const std::string & parameters)
{
  if (parameters.empty())
  {
    ...
    std::cout << "------------------------------------------------------------------------";
    std::cout << "7) 'set decode <filename>' - set the encoding table for decoding the file\n";
    std::cout << "8) 'combine <filename1> <filename2> <filename3> ' - merge data from filename1 and filename2 into filename3\n";
    std::cout << "9) 'decode <filename1> <filename2>' - decode the text contained in filename1 using the encoding table specified in filename2\n";
    std::cout << "------------------------------------------------------------------------";
